import { ArrowLeft, Check, MessageCircle } from "lucide-react";
import { Link, useParams } from "react-router-dom";
import { activeClient } from "../config/client";
import ProductImage from "../features/products/components/ProductImage";
import products from "../features/products/data/products";
import { type Product as ProductType } from "../features/products/types";

const Product = () => {
  const { id } = useParams();
  const productItems: ProductType[] = products;

  const product = productItems.find((currentProduct) => currentProduct.id === id);

  if (!product) {
    return (
      <div className="surface-card rounded-2xl p-6">
        <p className="text-muted">Producto no encontrado.</p>
        <Link to="/catalog" className="primary-link mt-3 inline-block text-sm font-semibold">
          Volver al catalogo
        </Link>
      </div>
    );
  }

  const whatsappLink = `https://wa.me/${activeClient.whatsappNumber}?text=${encodeURIComponent(
    `Hola, me interesa ${product.name}`
  )}`;

  return (
    <div className="space-y-6">
      <nav className="surface-soft rounded-2xl px-4 py-3">
        <Link to="/catalog" className="primary-link inline-flex items-center gap-2 text-sm font-semibold">
          <ArrowLeft size={16} aria-hidden="true" />
          Volver al catalogo
        </Link>
      </nav>

      <section className="surface-card rounded-3xl p-4 md:p-6">
        <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
          <div className="surface-soft rounded-2xl p-3 md:p-4">
            <ProductImage src={product.image} className="h-72 w-full rounded-xl object-cover sm:h-96 lg:h-[520px]" alt={product.name} />
          </div>

          <div className="flex h-full flex-col">
            {product.category && (
              <p className="text-xs font-semibold uppercase tracking-wide text-muted">{product.category}</p>
            )}
            <h1 className="mt-2 text-3xl font-bold">{product.name}</h1>
            <p className="mt-4 text-sm text-muted md:text-base">{product.description}</p>

            <div className="mt-6">
              <a
                href={whatsappLink}
                target="_blank"
                rel="noreferrer"
                className="whatsapp-button inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition hover:opacity-90"
              >
                <MessageCircle size={18} aria-hidden="true" />
                Consultar por WhatsApp
              </a>
            </div>
          </div>
        </div>
      </section>

      <section className="surface-card rounded-3xl p-6 md:p-8">
        <h2 className="text-lg font-semibold">Caracteristicas</h2>
        <ul className="mt-4 space-y-2 text-sm text-muted md:text-base">
          {product.technicalSpecs.map((specification) => (
            <li key={specification} className="flex items-start gap-2">
              <Check size={16} className="mt-0.5" aria-hidden="true" />
              <span>{specification}</span>
            </li>
          ))}
        </ul>
      </section>
    </div>
  );
};

export default Product;
